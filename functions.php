...
function get_rss_news($limit = 3) {
	...
	if ($rs = $rss->get($rssurl)) { 
		...
		$ret = '<li class="media">
					...
					<p class="mt-1 mb-2">We cannot fetch the latest announcements. Please join us at <a href="http://blankrefer.com/?https://www.phpsugar.com/blog/" target="_blank">www.phpsugar.com</a> to stay up-to-date.</p>
					</div>
				</li>';
	} 
	return $ret;
...
function show_pm_notes() {
	...
	if (version_compare($official_version, $config['version']) == 1) 
	{
		...
		$txt_notes[$i]['desc']  = '<strong>PHP Melody '.$official_version.'</strong> is available for download.<br /> Access your <a href="http://blankrefer.com/?https://www.phpsugar.com/customer/" target="_blank">Customer Account</a> to download the update pack. ';
